...
public abstract class ObjectEditor extends Table {
    ...
    public VisSlider createSlider(String label, float minimum, float maximum, float[] snapValues, float initialValue,
            ChangeListener changeListener) {
        return createSlider(label, minimum, maximum, snapValues, 10, initialValue, changeListener);
    }

    public VisSlider createSlider(String label, float minimum, float maximum, float[] snapValues, float threshold,
            float initialValue, ChangeListener changeListener) {
        if (!label.isEmpty()) {
    ...
    public VisTextButton createColorButton(String text, final Color defaultColor,
            final ColorPickerAdapter colorPickerAdapter) {
